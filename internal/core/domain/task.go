...
func (t *Task) Validate() error {
	...
	if t.Completed {
		if t.CompletedAt == nil {
			return fmt.Errorf("`CompletedAt` can't be nil if `Completed` is true: %w", core_errors.ErrInvalidArgument)
		}
		...
		if t.CompletedAt != nil {
			return fmt.Errorf("`CompletedAt`must be nil if `Completed` is false: %w", core_errors.ErrInvalidArgument)
		}
	}
